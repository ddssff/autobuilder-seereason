...
-}
  ...
  _network26 <- hackage (Just "2.6.3.5") "network" >>= debianize [] >>= inGroups ["platform", "ghcjs-comp"] >>= ghcjs_only
  ...
  _stm <- hackage (Just "2.4.5.0") "stm" >>= debianize [] >>= inGroups ["platform"] >>= ghcjs_also
