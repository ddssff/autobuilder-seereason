...
import Debian.AutoBuilder.Details.Common (broken, broken2, ghcjs_also, ghcjs_only, git', gitrepo, gitrepo2, hack, {-noProf,-} noTests, putSrcPkgName, Reason(Reason), replacement, skip, skip0, skip2, tflag, qflag, pflag, substitute, wflag, TSt)
...
commonTargets = do
  ...
  _aeson <- hackage (Just "1.4.2.0") "aeson" >>=
            debianize [] >>=
  ...
  _attoparsec <- hackage (Just "0.13.2.2") "attoparsec" >>= debianize [] >>= inGroups ["important", "ghcjs-comp"] >>= ghcjs_also
  -- _attoparsec_enumerator <- hackage (Just "0.3.4") "attoparsec-enumerator" >>= debianize []
  _attoparsec_iso8601 <- hackage (Just "1.0.1.0") "attoparsec-iso8601" >>= debianize [] >>= ghcjs_also
  ...
  _base_compat <- hackage (Just "0.10.5") "base-compat" >>= debianize [] >>= ghcjs_also
  -- Requires base-compat >= 0.10, required by text-show-3.7.4
  _base_compat_batteries <- hackage (Just "0.10.5") "base-compat-batteries" >>= debianize [] >>= ghcjs_also
  ...
  _basement <- hackage (Just "0.0.8") "basement" >>= debianize [] >>= ghcjs_also
  _bifunctors <-  (hackage (Just "5.5.3") "bifunctors") >>= debianize [] >>= inGroups ["kmett"] >>= ghcjs_also
  ...
  _bitarray <- hackage (Just "0.0.1.1") "bitarray" >>= flag (P.DebVersion "0.0.1.1-3") >>= debianize [] >>= ghcjs_also
  ...
  _blaze_markup <- hackage (Just "0.8.2.2") "blaze-markup" >>= debianize [] >>= inGroups ["happstack", "important"] >>= ghcjs_also
  ...
  _clckwrks <-
    createPackage (Debianize'' ((DataFiles {-(DataFiles
                                              (Hackage "clckwrks")
  ...
  _clckwrks_plugin_media  <- gitrepo "clckwrks-plugin-media" >>= {-patch $(embedFile "patches/clckwrks-plugin-media-locktag.diff") >>=-} flag (P.BuildDep "hsx2hs") >>= debianize [] >>= inGroups ["clckwrks", "important", "acid-state"]
  _clckwrks_plugin_page   <- gitrepo "clckwrks-plugin-page" >>= {-patch $(embedFile "patches/clckwrks-plugin-page-locktag.diff") >>=-} flag (P.BuildDep "hsx2hs") >>= debianize [] >>= inGroups ["clckwrks", "important", "acid-state"]
  ...
  _conduit <- hackage (Just "1.3.0.3") "conduit" >>= debianize [] >>= inGroups ["conduit", "important", "markdown"] >>= ghcjs_also
  _conduit_extra <- hackage (Just "1.3.0") "conduit-extra" >>= debianize [] >>= inGroups ["conduit", "important", "markdown"] >>= ghcjs_also
  ...
  _control_monad_free <-  (hackage (Just "0.6.2") "control-monad-free") >>= debianize []
  ...
  _crypto_api <- hackage (Just "0.13.2") "crypto-api" >>= flag (P.DebVersion "0.13.2-7build4") >>= debianize [] >>= ghcjs_also
  ...
  _css_text <- hackage (Just "0.1.3.0") "css-text" >>= inGroups ["css"] >>= debianize [] >>= inGroups ["important"] >>= ghcjs_also
  ...
  _data_accessor <- hackage (Just "0.2.2.8") "data-accessor" >>= debianize []
  _data_accessor_template <-  (hackage (Just "0.2.1.16") "data-accessor-template") >>= debianize []
  _data_binary_ieee754 <- hack (Just "0.4.4") "data-binary-ieee754" >>= flag (P.DebVersion "0.4.4-7build1")
  _data_default <- hackage (Just "0.7.1.1") "data-default" >>= flag (P.DebVersion "0.7.1.1-2build2") >>= debianize [] >>= ghcjs_also
  _data_default_class <- hackage (Just "0.1.2.0") "data-default-class" >>= flag (P.DebVersion "0.1.2.0-2build1") >>= debianize [] >>= ghcjs_also
  _data_default_instances_base <- hackage (Just "0.1.0.1") "data-default-instances-base" >>= flag (P.DebVersion "0.1.0.1-2build1") >>= debianize [] >>= ghcjs_also
  _data_default_instances_containers <- hackage (Just "0.0.1") "data-default-instances-containers" >>= flag (P.DebVersion "0.0.1-7build1") >>= debianize [] >>= ghcjs_also
  _data_default_instances_dlist <- hackage (Just "0.0.1") "data-default-instances-dlist" >>= flag (P.DebVersion "0.0.1-7build2") >>= debianize [] >>= ghcjs_also
  _data_default_instances_old_locale <- hackage (Just "0.0.1") "data-default-instances-old-locale" >>= flag (P.DebVersion "0.0.1-7build1") >>= debianize [] >>= ghcjs_also
  ...
  _debian_haskell <- {-hackage (Just "3.93.2") "debian"-} git "https://github.com/ddssff/debian-haskell" [] >>= debianize [] >>= flag (P.RelaxDep "cabal-debian") >>= inGroups ["autobuilder-group", "important"]
  ...
  --_diff <- hackage (Just "0.3.4") "Diff" >>= debianize [] >>= inGroups ["pretty"] >>= ghcjs_also
  -- _dictionary_sharing <- hackage Nothing "dictionary-sharing" >>= debianize []
  _diff <- git "https://github.com/seereason/Diff" [] >>= debianize [] >>= inGroups ["pretty"] >>= ghcjs_also
  ...
  _doctest <- hackage (Just "0.16.0.1") "doctest" >>= debianize [] {->>= ghcjs_also-}
  ...
  _ekg_core <- hackage (Just "0.1.1.6") "ekg-core" >>= debianize []
  ...
  _email_validate <- hackage (Just "2.3.2.9") "email-validate" >>= debianize [] >>= inGroups ["important"]
  ...
  _entropy <- hackage (Just "0.4.1.4") "entropy" >>= debianize [] >>= ghcjs_also
  -- _enumerator <- hackage (Just "0.4.20") "enumerator" >>= flag (P.DebVersion "0.4.20-6build2") >>= debianize []
  _erf <- hackage (Just "2.0.0.0") "erf" >>= flag (P.DebVersion "2.0.0.0-11build1") >>= debianize [] >>= ghcjs_also
  ...
  _feed <- hackage (Just "1.0.1.0") "feed" >>= debianize [] >>= inGroups ["acid-state"]
  _fgl <- hackage (Just "5.7.0.1") "fgl" >>= debianize [] >>= inGroups ["platform"] >>= ghcjs_also
  ...
  _filemanip <- git "https://github.com/ddssff/filemanip" [] >>= debianize []
  _filemanip_extra <- git "https://github.com/seereason/filemanip-extra" [] >>= debianize [] >>= inGroups ["important"]
  ...
  _fmlist <- hackage (Just "0.9.2") "fmlist" >>= debianize [] >>= ghcjs_also
  ...
  _foundation <- hackage (Just "0.0.21") "foundation" >>= debianize [] >>= ghcjs_also
  ...
  _generic_deriving <- hackage (Just "1.12.2") "generic-deriving" >>= debianize [] >>= ghcjs_also
...
-}
  ...
  _happstack_scaffolding <-  (git "https://github.com/seereason/happstack-scaffolding" [] >>= flag (P.BuildDep "hsx2hs")) >>= debianize [] >>= inGroups ["seereason", "important", "acid-state", "happstack", "appraisalscribe"]
  ...
  _happstack_util <- git "https://github.com/seereason/happstack-util" [] >>=
                     ...
                     debianize [] >>=
                     inGroups ["happstack", "important"]
  _happstack_websockets <- git "https://github.com/Happstack/happstack-websockets" [] >>= debianize [] >>= inGroups ["happstack"]
  _harp <-  (git "https://github.com/seereason/harp" []) >>= debianize []
  _hashable <-  (hackage (Just "1.2.7.0") "hashable") >>= {-flag (P.DebVersion "1.2.7.0-5") >>=-} debianize [] >>= inGroups ["ghcjs-comp"] >>= ghcjs_also
  ...
  _haskell_either <- hackage (Just "5.0.1") "either" >>= debianize [] >>= inGroups ["kmett"]
  ...
  _haskell_src_exts <- hackage (Just "1.21.0") "haskell-src-exts" >>=
                       debianize [] >>=
                       flag (P.BuildDep "happy") >>=
                       inGroups ["important"] >>= ghcjs_also
  -- This goes with haskell-src-exts-1.18.*
  -- _haskell_src_exts_simple <- hackage "haskell-src-exts-simple" >>= debianize []
  _haskell_src_meta <- -- hackage (Just "0.8.0.3") "haskell-src-meta"
                       git "https://github.com/galenhuntington/haskell-src-meta" [] >>=
  ...
  _haTeX <- hackage (Just "3.19.0.0") "HaTeX" >>=
            -- git "https://github.com/seereason/HaTeX" [Branch "linebreak"] >>= -- adds mapLaTeXT, fix \\ format
            patch $(embedFile "patches/HaTeX.diff") >>=
            debianize [] >>= ghcjs_also
  _haXml <- hackage (Just "1.25.5") "HaXml" >>= debianize [] >>= inGroups ["pretty"]
  ...
  _hint <- hackage (Just "0.9.0") "hint" >>= debianize []
  ...
  _hsemail <- hackage (Just "2") "hsemail" >>= flag (P.DebVersion "2-1") >>= debianize [] -- (rel release [] [P.DebVersion "1.7.1-2build2"])
  _hslogger <- hackage (Just "1.2.12") "hslogger" >>= debianize [] >>= inGroups ["important"] >>= ghcjs_also
  ...
  _hspec <- hackage (Just "2.6.1") "hspec" >>= debianize [] >>= ghcjs_also
  _hspec_core <- hackage (Just "2.6.1") "hspec-core" >>= debianize [] >>= ghcjs_also
  _hspec_discover1 <- hackage (Just "2.6.1") "hspec-discover" >>= flag (P.CabalDebian ["--default-package", "hspec-discover"]) >>= debianize []
  _hspec_discover2 <- hackage (Just "2.6.1") "hspec-discover" >>= flag (P.CabalDebian ["--default-package", "hspec-discover-ghcjs"]) >>= debianize [] >>= ghcjs_only
  _hspec_expectations <- hackage (Just "0.8.2") "hspec-expectations" >>= flag (P.DebVersion "0.8.2-1build1") >>= debianize [] >>= ghcjs_also
  _hspec_meta <- hackage (Just "2.6.0") "hspec-meta" >>= debianize []
  _hsSyck <-  (hackage (Just "0.53") "HsSyck") >>= debianize []
  _hStringTemplate <- hackage (Just "0.8.7") "HStringTemplate" >>= debianize []
  (_hsx2hs, _) <- hackage (Just "0.14.1.5") "hsx2hs" >>=
  ...
  _htf <- hackage (Just "0.13.2.5") "HTF" >>= flag (P.BuildDep "cpphs") >>= debianize []
  ...
  _http_api_data <- hackage (Just "0.4") "http-api-data" >>= debianize [] >>= ghcjs_also
  ...
  _http_types <- hackage (Just "0.12.2") "http-types" >>= debianize [] >>= inGroups ["happstack", "important"] >>= ghcjs_also -- web-routes specifies << 0.9
  ...
  _integer_logarithms <- hackage (Just "1.0.2.2") "integer-logarithms" >>= debianize [] >>= inGroups ["ghcjs-comp"] >>= ghcjs_also
  _intervals <- hackage (Just "0.8.1") "intervals" >>= flag (P.DebVersion "0.8.1-1build1") >>= debianize []
  _invariant <- hackage (Just "0.5.1") "invariant" >>= debianize [] >>= ghcjs_also
  ...
  -- _ircbot <- hackage Nothing "ircbot" >>= debianize [] >>= inGroups ["happstack", "important"]
  _ipynb <- hackage Nothing "ipynb" >>= debianize [] >>= inGroups ["pandoc"]
  ...
  _ixset_typed <- hackage (Just "0.4.0.1") "ixset-typed" >>= debianize [] >>= inGroups [ "authenticate", "important"] >>= ghcjs_also -- dependency of happstack-authenticate-2
...
-}
  ...
  -- _json <- hackage (Just "0.9.2") "json" >>= patch $(embedFile "patches/json.diff") >>= debianize [] >>= inGroups ["seereason", "important", "appraisalscribe"] >>= ghcjs_also -- darcs "haskell-json" (repo ++ "/haskell-json")
  _juicyPixels <- hackage (Just "3.3.3") "JuicyPixels" >>= debianize [] >>= inGroups ["happstack", "important"] >>= ghcjs_also
  ...
  -- patch removes fail dependency
  _kan_extensions <- hackage (Just "5.2") "kan-extensions" >>= debianize [] >>= inGroups ["kmett"] >>= ghcjs_also
  ...
  _lens_family_core <-  (hackage (Just "1.2.3") "lens-family-core") >>= debianize []
  _lens_family <-  (hackage (Just "1.2.3") "lens-family") >>= debianize []
  ...
  -- _libsystemd_journal <- hackage (Just "1.4.3") "libsystemd-journal" >>= flag (P.BuildDep "libsystemd-dev") >>= flag (P.CabalDebian [ "--dev-dep", "libsystemd0" ]) >>= debianize []
  _libv8 <- apt "sid" "libv8-3.14" >>= skip (Reason "Use standard")
  _lifted_async <- hackage (Just "0.10.0.3") "lifted-async" >>= debianize [] >>= inGroups ["ghcjs-comp"]
  ...
  _listLike <- git "https://github.com/JohnLato/ListLike" [] >>= flag (P.CabalDebian ["--cabal-flags", "safe"]) >>= debianize [] >>= inGroups ["pretty"] >>= ghcjs_also
  _list_tries <-  hackage (Just "0.6.6") "list-tries" >>= debianize [] >>= inGroups ["happstack", "important"] -- version 0.5.2 depends on dlist << 0.7
  ...
  _logic_TPTP <-  (hackage (Just "0.4.6.0") "logic-TPTP") >>= debianize []
  ...
  _lucid <- hackage (Just "2.9.11") "lucid" >>= debianize [] >>= inGroups ["appraisalscribe", "important"] >>= ghcjs_also
  ...
  _magic <- hackage (Just "1.1") "magic" >>= flag (P.DebVersion "1.1-6build1") >>= flag (P.DevelDep "libmagic-dev") >>= debianize []
  ...
  _markdown <-  (hackage (Just "0.1.17.4") "markdown" {- >>= patch $(embedFile "patches/markdown.diff") -}) >>= debianize [] >>= inGroups ["happstack", "important", "markdown"] >>= ghcjs_also
  ...
  _multiset <- hackage (Just "0.3.4.1") "multiset" >>= debianize []
  ...
  _network_arbitrary <- hackage (Just "0.4.0.2") "network-arbitrary" >>= debianize []
  ...
  -- _openGLRaw <- hackage (Just "3.2.7.0") "OpenGLRaw" >>= flag (P.DebVersion "3.2.7.0-1") >>= inGroups ["gl"] >>= flag (P.DevelDep "libgl1-mesa-dev") >>= debianize []
  ...
  _pandoc_types <- hackage (Just "1.17.5.4") "pandoc-types" >>= debianize [] >>= inGroups ["appraisalscribe", "important"] >>= ghcjs_also
  _pango <-  (hackage (Just "0.13.1.1") "pango") >>= debianize [] >>= skip (Reason "see cairo")
  _parallel <- hackage (Just "3.2.2.0") "parallel" >>= debianize [] >>= inGroups ["platform"] >>= ghcjs_also
  ...
  _parsers <- hackage (Just "0.12.9") "parsers" >>= debianize [] >>= ghcjs_also
  ...
  _pcre_light <- hackage (Just "0.4.0.4") "pcre-light" >>=
                 ...
                 ghcjs_also
  -- Requires gcj-jdk, which is not available in bionic.  But
  -- pdftk-java supplies it and is available in cosmic.
  ...
  _polyparse <- hackage (Just "1.12.1") "polyparse" >>= debianize [] >>= ghcjs_also
  ...
  _pureMD5 <- hackage (Just "2.1.3") "pureMD5" >>= flag (P.DebVersion "2.1.3-3build4") >>= debianize [] >>= inGroups ["authenticate", "important"] >>= ghcjs_also
  ...
  _quickCheck <- hackage (Just "2.12.6.1") "QuickCheck" >>= flag (P.BuildDep "libghc-random-prof") >>= debianize [] >>= inGroups ["platform"] >>= ghcjs_also
  ...
  _regex_compat <- hackage (Just "0.95.1") "regex-compat" >>= flag (P.DebVersion "0.95.1-10build1") >>= debianize [] >>= inGroups ["platform"]
  ...
  _regex_posix <- hackage (Just "0.95.2") "regex-posix" >>= flag (P.DebVersion "0.95.2-9build1") >>= debianize [] >>= inGroups ["platform"]
  ...
  -- _reified_records =  (hackage (Just "0.2.2") "reified-records" >>= patch $(embedFile "patches/reified-records.diff")) >>= debianize []
  ...
  _safecopy <- hackage (Just "0.9.4.3") "safecopy" >>= flag (P.DebVersion "0.9.4.3-1") >>= debianize [] >>= inGroups ["appraisalscribe"] >>= ghcjs_also
  _safeSemaphore <-  (hackage (Just "0.10.1") "SafeSemaphore") >>= flag (P.DebVersion "0.10.1-7build1") >>= debianize [] >>= inGroups ["happstack", "important"]
  _sandi <- hackage (Just "0.4.3") "sandi" >>= debianize [] -- replaces dataenc
  ...
  _seereason_base <- git "https://github.com/seereason/seereason-base" [] >>= debianize [] >>= inGroups ["seereason", "important", "acid-state", "appraisalscribe"]
  ...
  _servant <- hackage (Just "0.15") "servant" >>= debianize [] >>= inGroups ["servant"] >>= ghcjs_also
  ...
  _setlocale <- hackage (Just "1.0.0.8") "setlocale" >>= debianize []
  ...
  (_ghc_skylighting_core, _ghcjs_skylighting_core) <- hackage (Just "0.7.7") "skylighting-core" >>= patch $(embedFile "patches/skylighting-core.diff") >>= debianize [] >>= inGroups ["pandoc"] >>= ghcjs_also
  (_ghc_skylighting, _ghcjs_skylighting) <- hackage (Just "0.7.7") "skylighting" >>= patch $(embedFile "patches/skylighting.diff") >>= debianize [] >>= flag (P.DevelDep "libpcre2-dev") >>= inGroups ["pandoc"] >>= ghcjs_also
  ...
  _sop_core <- hackage (Just "0.4.0.0") "sop-core" >>= debianize [] >>= ghcjs_also
  ...
  _stateVar <-  (hackage (Just "1.1.1.1") "StateVar") >>= debianize [] >>= ghcjs_also
  ...
  _stm <- hackage (Just "2.5.0.0") "stm" >>= debianize [] >>= inGroups ["platform", "ghcjs-comp"] >>= ghcjs_only
  ...
  _tasty <-  (hackage (Just "1.2") "tasty") >>= debianize [] >>= ghcjs_also
  ...
  _test_framework_quickcheck2 <- hackage (Just "0.3.0.5") "test-framework-quickcheck2" >>= debianize [] >>= ghcjs_also
  ...
  _texmath <- hackage (Just "0.11.1.2") "texmath" >>= debianize [] >>= inGroups ["appraisalscribe", "important"] >>= ghcjs_also
  ...
  _th_desugar <- hackage (Just "1.9") "th-desugar" >>= debianize [] >>= inGroups ["th-path", "important"] >>= ghcjs_also
  ...
  _th_orphans <- hackage (Just "0.13.6") "th-orphans" >>= debianize [] >>= inGroups ["th-path", "important"] >>= ghcjs_also
  ...
  _time_locale_compat <- hackage (Just "0.1.1.5") "time-locale-compat" >>= debianize [] >>= inGroups ["happstack", "important"] >>= ghcjs_also
  ...
  _transformers_base <- hackage (Just "0.4.5.2") "transformers-base" >>= debianize [] >>= inGroups ["ghcjs-comp"] >>= ghcjs_also
  _transformers_compat <- hackage (Just "0.6.2") "transformers-compat" >>= {-flag (P.DebVersion "0.6.2-1") >>=-} debianize [] >>= inGroups ["ghcjs-comp"] >>= ghcjs_also
  ...
  _unixutils <- hackage Nothing "Unixutils" >>= flag (P.DebVersion "1.54.1-4build4") >>= debianize [] >>= inGroups ["important"] >>= ghcjs_also
  ...
  -- _urlencoded <- hackage (Just "0.4.2.0") "urlencoded" >>= debianize []
  ...
  -- _utf8_string <- apt "bionic" "haskell-utf8-string" >>= flag (P.RelaxDep "hscolour") >>= flag (P.RelaxDep "cpphs") >>= debianize [] >>= ghcjs_also
  _utf8_string <- hackage Nothing "utf8-string" >>= flag (P.DebVersion "1.0.1.1-4build1") >>= debianize [] >>= ghcjs_also
  _utility_ht <- hackage (Just "0.0.14") "utility-ht" >>= flag (P.DebVersion "0.0.14-1") >>= debianize [] >>= ghcjs_also
  _uuid <- git "https://github.com/ddssff/uuid" [] >>= flag (P.DebVersion "1.3.13-2build3") >>= cd "uuid" >>= debianize [] >>= inGroups ["uuid"] >>= ghcjs_also
  _uuid_orphans <- git "https://github.com/seereason/uuid-orphans" [] >>= debianize [] >>= inGroups ["clckwrks", "important"] >>= inGroups ["uuid"] >>= ghcjs_also
  ...
  _vault <- hackage (Just "0.3.1.2") "vault" >>= debianize [] >>= ghcjs_also
  -- _vc_darcs <- darcs ("http://src.seereason.com/vc-darcs")
  _vc_git_dired <- git "https://github.com/ddssff/vc-git-dired" []
  _vector <- hackage (Just "0.12.0.2") "vector" >>= debianize [] >>= ghcjs_also
  _vector_algorithms <- hackage (Just "0.8.0.1") "vector-algorithms" >>= debianize [] >>= ghcjs_also
  _vector_binary_instances <- hackage (Just "0.2.5.1") "vector-binary-instances" >>= debianize [] >>= ghcjs_also
...
-}
  ...
  _warp <- hackage (Just "3.2.25") "warp" >>= debianize [] {->>= ghcjs_also-}
  _wcwidth <- hackage (Just "0.0.2") "wcwidth" >>= debianize []
...
-}
  ...
  _websockets <- hackage (Just "0.12.5.2") "websockets" >>= patch $(embedFile "patches/websockets.diff") >>= debianize [] >>= inGroups ["websockets"] >>= ghcjs_also
  ...
  _xml_conduit <- hackage (Just "1.8.0") "xml-conduit" >>= debianize [] >>= inGroups ["conduit", "authenticate", "important", "markdown"] >>= ghcjs_also
  ...
  _xml_types <- hackage (Just "0.3.6") "xml-types" >>= flag (P.DebVersion "0.3.6-5build2") >>= debianize [] >>= inGroups ["markdown"] >>= ghcjs_also
  _xss_sanitize <- hackage (Just "0.3.6") "xss-sanitize" >>= debianize [] >>= inGroups ["important"] >>= ghcjs_also
  ...
  _zip_archive <- hackage (Just "0.4") "zip-archive" >>= flag (P.BuildDep "zip") >>= debianize [] >>= ghcjs_also
