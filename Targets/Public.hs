...
{-  , P.Package { P.name = "haskell-deepseq"
  ...
  -- added.
    ...
    , debianize "json" Newest [P.Patch (B.pack (unlines genericPatch)), P.Patch (B.pack (unlines bytestringPatch))]
...
genericPatch =
    [ "diff -ru old/json.cabal new/json.cabal"
    , "--- old/json.cabal\t2011-09-19 21:28:40.000000000 -0700"
    , "+++ new/json.cabal\t2012-02-01 14:16:23.024178366 -0800"
    , "@@ -90,7 +90,7 @@"
    , " "
    , "   if flag(split-base)"
    , "     if flag(generic)"
    , "-      build-depends:    base >=4 && <5, syb >= 0.3.3"
    , "+      build-depends:    base >=4 && <5, syb >= 0.3.3, text"
    , " "
    , "       exposed-modules:  Text.JSON.Generic"
    , "       Cpp-Options:      -DBASE_4"
    , "diff -ru json-0.5.orig/tests/GenericTest.hs json-0.5/tests/GenericTest.hs"
    , "--- json-0.5.orig/tests/GenericTest.hs\t2011-09-19 21:28:40.000000000 -0700"
    , "+++ json-0.5/tests/GenericTest.hs\t2012-02-01 14:16:23.024178366 -0800"
    , "@@ -3,6 +3,9 @@"
    ...
    , "+#if (BASE_MAJOR_VERSION >= 4)"
    , "+    testJSON (Text.pack \"Hello, World\\n\"),"
    , "+#endif"
    , "     testJSON (Nothing :: Maybe Int),"
    , "     testJSON (Just \"aa\"),"
    , "     testJSON [],"
    , "diff -ru json-0.5.orig/tests/QC.hs json-0.5/tests/QC.hs"
    , "--- json-0.5.orig/tests/QC.hs\t2011-09-19 21:28:40.000000000 -0700"
    , "+++ json-0.5/tests/QC.hs\t2012-02-01 14:16:23.024178366 -0800"
    , "@@ -27,6 +27,9 @@"
    ...
    , "+#if (BASE_MAJOR_VERSION >= 4)"
    , "+        ,(\"[Text.Text]\",    p (test :: T [Text.Text]                  ))"
    , "+#endif"
    , " "
    , "     ])"
    , "         -- containers"
    , "diff -ru json-0.5.orig/tests/QuickCheckUtils.hs json-0.5/tests/QuickCheckUtils.hs"
    , "--- json-0.5.orig/tests/QuickCheckUtils.hs\t2011-09-19 21:28:40.000000000 -0700"
    , "+++ json-0.5/tests/QuickCheckUtils.hs\t2012-02-01 14:16:23.024178366 -0800"
    , "@@ -44,6 +44,10 @@"
    ...
    , "+  coarbitrary s = coarbitrary (Text.unpack s)"
    , "+#endif"
    , "diff -ru json-0.5.orig/Text/JSON/Generic.hs json-0.5/Text/JSON/Generic.hs"
    , "--- json-0.5.orig/Text/JSON/Generic.hs\t2011-09-19 21:28:40.000000000 -0700"
    , "+++ json-0.5/Text/JSON/Generic.hs\t2012-02-01 14:16:23.024178366 -0800"
    , "@@ -1,4 +1,4 @@"
    ...
    , "+#if (BASE_MAJOR_VERSION >= 4)"
    , "+         `extQ` (showJSON :: T Text.Text)"
    , "+#endif"
    , "   where"
    , "         -- Lists are simply coded as arrays."
    , "         jList vs = JSArray $ map toJSON vs"
    , "diff -ru json-0.5.orig/Text/JSON.hs json-0.5/Text/JSON.hs"
    , "--- json-0.5.orig/Text/JSON.hs\t2011-09-19 21:28:40.000000000 -0700"
    , "+++ json-0.5/Text/JSON.hs\t2012-02-01 14:16:23.024178366 -0800"
    , "@@ -64,6 +64,10 @@"
    , " "
    , " import qualified Data.ByteString.Char8 as S"
    , " import qualified Data.ByteString.Lazy.Char8 as L"
    ...
    , "+-- Data.Text"
    , "+"
    , "+instance JSON Text.Text where"
    , "+  showJSON = encJSString unpack"
    , "+  readJSON = decJSString \"Text\" (return . pack)"
    , "+#endif"
    , "+"
    , " -- -----------------------------------------------------------------"
    , " -- Instance Helpers" ]
...
bytestringPatch =
    [ "--- old/tests/GenericTest.hs\t2012-02-01 14:16:23.000000000 -0800"
    , "+++ new/tests/GenericTest.hs\t2012-02-01 14:20:00.903847032 -0800"
    , "@@ -6,6 +6,8 @@"
    , " #if (BASE_MAJOR_VERSION >= 4)"
    , " import qualified Data.Text as Text"
    , " #endif"
    ...
    , "     testJSON (Text.pack \"Hello, World\\n\"),"
    , " #endif"
    , "+    testJSON (L.pack \"Hello, World\\n\"),"
    , "+    testJSON (B.pack \"Hello, World\\n\"),"
    , "     testJSON (Nothing :: Maybe Int),"
    , "     testJSON (Just \"aa\"),"
    , "     testJSON []," ]
