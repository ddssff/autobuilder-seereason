    ...
    where
      ...
      devscripts =
        P.Package { P.name = "haskell-devscripts"
                  , P.spec = ghc release (Patch (Apt "precise" "haskell-devscripts") $(embedFile "patches/haskell-devscripts-0.8.12.diff")) -- Apt "sid" "haskell-devscripts"
...
platform release =
    ...
    -- debianize "directory" []
    , debianize "stm" []
    , ghc release (apt "sid" "haskell-zlib" [])
                      (debianize "zlib" [])
    , debianize "mtl" []
